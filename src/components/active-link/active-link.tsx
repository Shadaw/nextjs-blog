...
export function ActiveLink({ children, href, ...rest }: ActiveLink) {
  ...
  return (
    <Link href={href} className={cn('text-sm font-medium transition-colors hover:text-blue-500', isCurrentPath ? 'text-blue-500' : 'text-muted-foreground')} {...rest}>{children}</Link>
  )
}